<div class = "content_text" >
	<br>

	<div class = "main_img center" >
		<img class = "shadow" src = "/resources/img/main_img/meta1.png" alt = "main_img" >
	</div><br>

	<p> Приветствую тебя странник, что то я впервые тебя здесь вижу.
		Трусливым у нас нет места.
		Чтобы выжить тебе нужно для начала найти убежище.
		Осмотрись и вибери подходящее место.
		Вижу ты уже получил приписку у нас, пошли за мной... </p>
	<br />

	<div class = "center">
		<a class="button_medium" href="/beginner/next">Идти</a>
	</div>
</div>